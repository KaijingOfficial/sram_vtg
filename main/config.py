...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--dset_name", type=str, choices=["qvhighlights", "charades", "didemo", "anet", "tvsum", "youtube", "summe", "ego4d", "qfvs", "video2gif", "coin", "hacs", "vlp", "videocc", "tacos"])
        ...
        parser.add_argument("--mlm_bsz", type=int, default=128, help="mini-batch size")
        parser.add_argument("--mlm_epoch", type=int, default=0, help="number of epochs to run")
        parser.add_argument("--mlm_lr", type=float, default=1e-3, help="learning rate")
        parser.add_argument("--mlm_warmup_steps", type=float, default=1, help="linear warmup scheme")
        parser.add_argument("--mlm_lr_drop", type=int, default=400, help="drop learning rate to 1/10 every lr_drop epochs")
        parser.add_argument("--mlm_lr_gamma", type=float, default=0.1, help="lr reduces the gamma times after the `drop' epoch")
        # ** Loss coefficients
        # *** boundary branch
        ...
        # *** video contrast
        parser.add_argument("--vid_contrast_coef", type=float, default=1., help="intra-video single modality contrast loss")

        
        # *** edl weight
        parser.add_argument("--edl_coef", type=float, default=1, help="edr loss weight")
        # *** reg weight
        parser.add_argument("--reg_coef", type=float, default=1e-1, help="edr regression weight")
                
        parser.add_argument("--mlm_coef", type=float, default=1e-1, help="")
        
        parser.add_argument("--con_coef", type=float, default=1e-1, help="")
        ...
        parser.add_argument("--tokenizer_path",type=str, default="clip-vit-base-patch32")
        ...
        parser.add_argument('--num_cross_layers', default=4, type=int,
                            help="Number of cross layers in the transformer")
        ...
        parser.add_argument('--cache_attention', action='store_true')
        ...
        parser.add_argument("--qfvs_vid_feature", type=str, default='fps1_224')
        ...
        parser.add_argument("--mlm_ratio", type=float, default=1.0)

        # geom loss
        parser.add_argument("--geom_coef", type=float, default=1.0) 
        parser.add_argument(
            "--geom_norm_mode", 
            type=str, 
            default='maxmin_batch', 
            choices=[
                'maxmin_video',
                'maxmin_batch',
                'activate',
        ...
        parser.add_argument('--geom_coef_list', nargs='+', type=float, help='coef of loss-au_acc, loss-eu_acc and loss-evi_mis')
        self.parser = parser
...
def setup_model(opt):
    ...
    model = importer.build_model(opt)
    criterion = importer.build_mlm_criterion(opt)
    ...
    param_dicts = [{"params": 
                    ...
                    + [p for n, p in criterion.named_parameters() if p.requires_grad]}]
...
def setup_model_mlm(opt, pretrained_model=None):
    """setup model/optimizer/scheduler and load checkpoints when needed"""
    logger.info("setup model/optimizer/scheduler")
    ...
    if int(opt.device) >= 0:
        ...
        model.to(opt.gpu_id)
        criterion.to(opt.gpu_id)
    param_dicts = [{"params": [p for n, p in model.named_parameters() if p.requires_grad],'lr': opt.mlm_lr},
                    {"params": [p for n, p in criterion.named_parameters() if p.requires_grad]}]
    optimizer = torch.optim.AdamW(param_dicts, lr=opt.lr, weight_decay=opt.wd)

    if opt.mlm_warmup_steps != -1 and opt.mlm_lr_drop > 0:
        lr_scheduler = WarmupStepLR(optimizer, warmup_steps=opt.mlm_warmup_steps[0], step_size=opt.mlm_lr_drop, gamma=opt.mlm_lr_gamma)
    
    elif opt.mlm_warmup_steps != -1:
        from transformers import get_constant_schedule_with_warmup
        lr_scheduler =  get_constant_schedule_with_warmup(optimizer, opt.mlm_warmup_steps[0])

    elif opt.mlm_lr_drop > 0:
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, opt.mlm_lr_drop, gamma=opt.mlm_lr_gamma)
...
def setup_model_grounding(opt, pretrained_model=None):
    """setup model/optimizer/scheduler and load checkpoints when needed"""
    logger.info("setup model/optimizer/scheduler")
    ...
    criterion = importer.build_grounding_criterion(opt)
