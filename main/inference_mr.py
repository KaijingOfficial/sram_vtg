...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        v = outputs["pred_v"]
        alpha = outputs["pred_alpha"]
        beta = outputs["pred_beta"]
        denominator = alpha-1
        aleatoric = beta / denominator
        epistemic = beta / ( denominator * v)
        # min_ep = torch.min(epistemic[:, None], dim=0).values
        # max_ep = torch.max(epistemic[:, None], dim=0).values
        # normlized_ep = (epistemic - min_ep) / (max_ep - min_ep)
        # min_al = torch.min(aleatoric[:, None], dim=0).values
        # max_al = torch.max(aleatoric[:, None], dim=0).values
        # normlized_al = (aleatoric - min_al) / (max_al - min_al)  
        
        pred_aleatoric = aleatoric.tolist()
        pred_epistemic = epistemic.tolist()
        ...
        # compose predictions
        for idx, (meta, spans, score,aleatoric,epistemic) in enumerate(zip(query_meta, pred_spans.cpu(), scores.cpu(),pred_aleatoric,pred_epistemic)):
            if opt.span_loss_type == "l1":
            ...
            # cur_v = v.tolist()
            # cur_alpha = alpha.tolist()
            # cur_beta = beta.tolist()
            ...
            cur_query_pred = dict(
                ...
                pred_aleatoric = aleatoric,
                pred_epistemic = epistemic
            )
