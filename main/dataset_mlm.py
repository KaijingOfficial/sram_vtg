...
from utils.basic_utils import load_jsonl, load_pickle, l2_normalize_np_array, flatten_list
...
class DatasetMR(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir, v_feat_dim, q_feat_dim,
                ...
                normalize_v=True, normalize_t=True, load_labels=True,
                clip_len=2, max_windows=5, span_loss_type="l1", txt_drop_ratio=0,
                use_cache=-1, fix_len=-1, add_easy_negative=1, easy_negative_only=-1,tokenizer=None,
                mlm_ratio=None):
    ...
    def __getitem__(self, index):
        ...
        model_inputs = dict()
        query_feat= self._get_query_feat_by_qid(meta["qid"])  # (Dq, ) or (Lq, Dq)
        
        if self.mlm_ratio is not None:
            token_ids = torch.Tensor(self.tokenizer.encode(meta["query"]))[:len(query_feat)]
            
            query_entities = meta["Entities"]
            index_to_mask = flatten_list([e[-1][0] for e in query_entities])
            index_to_mask = [x for x in index_to_mask if x < len(query_feat)]

            mask_index = torch.zeros(len(token_ids), dtype=torch.bool)
            
            random.shuffle(index_to_mask)
            num = min(round(self.mlm_ratio * len(index_to_mask)), 1)
            for index in index_to_mask[:num]:
                mask_index[index] = 1
            model_inputs["mask_index"] = mask_index
            model_inputs["mlm_label"] = torch.Tensor([value * int(bool_value) for value, bool_value in zip(token_ids, mask_index)])
            mask = (~mask_index).float()
            mask = mask.unsqueeze(-1)
            model_inputs["query_feat"] = query_feat * mask
        else:
        ...
        if self.load_labels:
            ...
            if 'saliency_scores' in meta.keys():
                ...
                model_inputs["saliency_scores"][meta["relevant_clip_ids"][:limit]] = torch.tensor(np.mean(np.array(meta["saliency_scores"][:limit]), -1))
                model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"] = \
                    self.get_saliency_labels(meta["relevant_clip_ids"], meta["saliency_scores"], ctx_l, 2)
            else:
                model_inputs["saliency_scores"] = model_inputs["timestamp_window"]
                model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"] = \
                    self.get_saliency_labels_sub_as_query(meta["relevant_windows"][0], ctx_l)  # only one gt
...
class DatasetHL(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dset_name == 'youtube':
            ...
            saliency_neg_labels = torch.Tensor([torch.argmin(saliency).item()])
        ...
        data = dict(
            ...
            saliency_neg_labels=saliency_neg_labels)
...
def prepare_batch_inputs_mr(batched_model_inputs, device, non_blocking=False, dtype=None):
    model_inputs = dict(
...
def prepare_batch_inputs_mr_mask(batched_model_inputs, device, non_blocking=False, dtype=None, mask_ratio=0.3, infer=False):
    
    src_txt = batched_model_inputs["query_feat"][0].to(device, non_blocking=non_blocking)
    masked_txt, masked_ids = random_mask_text_tokens(src_txt,infer=infer,mask_ratio=mask_ratio)
    model_inputs = dict(
        src_txt=masked_txt,
        src_txt_mask=batched_model_inputs["query_feat"][1].to(device, non_blocking=non_blocking),
        src_vid=batched_model_inputs["video_feat"][0].to(device, non_blocking=non_blocking),
    ...
    targets = {}
    targets['src_txt'] = src_txt
    targets['masked_index'] = masked_ids
    targets['timestamp'] = batched_model_inputs["timestamp"][0].to(device, non_blocking=non_blocking)
    targets['timestamp_mask'] = batched_model_inputs["timestamp"][1].to(device, non_blocking=non_blocking)
    targets['timestamp_window'] = batched_model_inputs["timestamp_window"][0].to(device, non_blocking=non_blocking)
    targets['span_labels_nn'] = batched_model_inputs["span_labels_nn"][0].to(device, non_blocking=non_blocking)
...
def prepare_batch_inputs_mr_mlm(batched_model_inputs, device, non_blocking=False, dtype=None, infer=False,tokenizer=None, mlm=False):
    ...
    targets = {}
    if mlm==True:
        targets['mlm_label'] = batched_model_inputs['mlm_label'][0].to(device, non_blocking=non_blocking)
        targets['masked_index'] = batched_model_inputs['mask_index'][0].to(device, non_blocking=non_blocking)
    targets['timestamp'] = batched_model_inputs["timestamp"][0].to(device, non_blocking=non_blocking)
    targets['timestamp_mask'] = batched_model_inputs["timestamp"][1].to(device, non_blocking=non_blocking)
    targets['timestamp_window'] = batched_model_inputs["timestamp_window"][0].to(device, non_blocking=non_blocking)
    targets['span_labels_nn'] = batched_model_inputs["span_labels_nn"][0].to(device, non_blocking=non_blocking)
...
def random_mask_text_tokens(tokens, infer=False, mask_ratio=0.3):
    """
    Randomly masks tokens in a given sequence tensor.

    Args:
    - tokens (torch.Tensor): Input sequence tensor of shape [batch_size, seq_length, hidden_dim].
    - mask_ratio (float): Ratio of tokens to be masked.

    Returns:
    - masked_tokens (torch.Tensor): Masked sequence tensor.
    - mask_index (torch.Tensor): Index tensor indicating the masked positions.
    """
    batch_size, seq_length, hidden_dim = tokens.size()
    
    # Calculate the number of tokens to mask
    num_to_mask = int(seq_length * mask_ratio)
    
    # Initialize mask index tensor
    mask_index = torch.zeros(batch_size, seq_length, dtype=torch.bool)
    
    # Randomly choose positions to mask
    for i in range(batch_size):
        indices_to_mask = random.sample(range(seq_length), num_to_mask)
        mask_index[i, indices_to_mask] = 1
    ...
    if infer:
        ...
        masked_tokens[mask_index] = 0
    
    return masked_tokens, mask_index
...
def prepare_batch_inputs_hl(batched_model_inputs, device='cuda', non_blocking=False):
    ...
    saliency_neg_labels = batched_model_inputs['saliency_neg_labels'][0].to(device, non_blocking=non_blocking)
    
    targets['saliency_scores'] = saliency
    targets['saliency_pos_labels'] = saliency_pos_labels.long()
    targets['saliency_neg_labels'] = saliency_neg_labels.long()
